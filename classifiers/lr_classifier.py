...
import numpy as np
import pandas as pd
from utils.config import EMOTION_MODEL_PATH, EMOTION_MODEL_PATH_INFO
...
clf.save_model(EMOTION_MODEL_PATH)

# 2. Using a pre-trained model for prediction:
clf = EmotionClassifier()
model_info = joblib.load(EMOTION_MODEL_PATH_INFO)
...
class EmotionClassifier:
    ...
    def evaluate_with_cross_validation(self, X, y, cv=5):
        pipeline = make_pipeline(
            ...
            LogisticRegression(
                C=1.0, class_weight="balanced", max_iter=5000, solver="liblinear"
            ),
        )
        scoring = {
            "accuracy": "accuracy",
    ...
    def tune_hyperparameters(self, X, y):
        ...
        grid = GridSearchCV(
            pipeline, param_grid, cv=3, scoring="f1_macro", verbose=1, n_jobs=-1
        )
        grid.fit(X, y)
    ...
    def save_model(self, filename=EMOTION_MODEL_PATH):
    ...
    def predict(self, text, model_info=None):
        ...
        top_emotions = [
            {
                "emotion": emotions_list[classes[i]]
                if classes[i] < len(emotions_list)
                else f"Unknown ({classes[i]})",
                "probability": proba[i],
            }
            for i in top_idxs
        ]
        return {
            "emotion": emotions_list[prediction]
            if prediction < len(emotions_list)
...
if __name__ == "__main__":
    ...
    print(f"\nSample: '{sample}'")
    print(
        f"Predicted Emotion: {prediction['emotion']} (Confidence: {prediction['confidence']:.2f})"
    )
    print("Top Emotions:")
